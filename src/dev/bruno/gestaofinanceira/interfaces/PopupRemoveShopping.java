 ...
 * @author bruno
 */
public class PopupRemoveShopping {
    ...
    public void initComponentsPopup(JFrame frame, JTable table, List<Shopping> list, DefaultTableModel modeloTable1) {
    ...
    private void LabelYesMouseClicked(java.awt.event.MouseEvent evt, JFrame frame, JTable table, List<Shopping> list, DefaultTableModel modeloTable1) {                                      
        int numb = table.getSelectedRow();
         
        Object id = table.getValueAt(numb,0);
         
        ShoppingDAO shoppingDAO = new ShoppingDAO();
        Shopping buy = shoppingDAO.readBuy(Integer.parseInt(id.toString()));
         
        JOptionPane.showMessageDialog(null, "Compra deletada com sucesso.", "Compra removida com sucesso!", JOptionPane.INFORMATION_MESSAGE);
        shoppingDAO.deleteBuy(buy);
        list.remove(buy);
        frame.dispose();

        Util.readTableShoppings(modeloTable1, list, true);
    }    
